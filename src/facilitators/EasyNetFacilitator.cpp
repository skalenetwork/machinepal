...
nlohmann::json EasyNetFacilitator::processSettleRequest(
    ...
    try {
        ...
        EthAddress toWalletAddress =
            paymentPayload->payload()->authorization()->to();
        EthAddress assetWalletAddress =
            EthAddress::parseFlexible( paymentReqs->asset() );
        EIP3009Value transferValue =
        ...
        auto result = db->processTransferRequest( fromWalletAddress,
            toWalletAddress, assetWalletAddress, transferValue, nonce, resource,
            "0.0.0.0", jsonInfo, transactionHash, chainId_,
        ...
        if ( result == EasyNetDb::TransferResult::TransferSuccess ) {
            ...
            SettlementResponse response( false,
...
EasyNetFacilitator::processVerifyRequestUnsafe(
    const nlohmann::json& verifyRequestJson, optional< string >& error,
    EasyNetDb& db ) const {
    ...
    auto assetWalletAddress =
        EthAddress::parseFlexible( paymentRequirements->asset() );
    ...
    auto senderBalanceOpt =
        db.getBalance( fromWalletAddress, assetWalletAddress );
    ...
    auto receiverBalanceOpt =
        db.getBalance( toWalletAddress, assetWalletAddress );
    ...
    if ( transferValue.value() > currentBalance ) {
        error =
            "InsufficientFunds: Balance lower than requested transfer amount";
        return { paymentPayload, paymentRequirements };
    }
...
nlohmann::json EasyNetFacilitator::processVerifyRequest(
    ...
    try {
        ...
        if ( error ) {
            VerifyResponse errorResponse( false, error.value(),
                fromWalletAddress.toDbString(), std::nullopt );

            return errorResponse.toJson();
        } else {
            VerifyResponse verifyResponse( true, std::nullopt,
                fromWalletAddress.toDbString(), std::nullopt );
