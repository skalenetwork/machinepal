...
variant< ptr< PaymentPayload >, HttpError > PaymentManager::decodeAndParsePayment(
    ...
    try {
        ...
        try {
            ...
            spdlog::error( "Failed to parse decoded X-PAYMENT header as JSON: {} {}", decoded,
                e.what() );
            return HttpError( ERR_BAD_REQUEST,
                "X-PAYMENT header is not valid JSON after base64 decoding" );
        }
...
std::optional< HttpError > PaymentManager::checkAgainstAlreadySettledPayments(
    const ptr< PaymentPayload >& paymentPayload,
    ...
    if (db->settledPaymentExists( paymentPayload, domain )) {
        ...
        spdlog::info(
            "Payment has already been spent: from={}, nonce={}, token={}, tokenAddress={}, chainId={}",
...
bool PaymentManager::lockPaymentAsBeingSettled( ptr< Authorization > authorization,
    ptr< EIP712Domain > domain ) {
    auto uniquePaymentKey = domain->uniquePaymentKey( authorization->from(),
        authorization->nonce() );
...
void PaymentManager::unlockPaymentAsBeingSettled( ptr< Authorization > authorization,
    ptr< EIP712Domain > domain ) {
    auto uniquePaymentKey = domain->uniquePaymentKey( authorization->from(),
        authorization->nonce() );
    lock_guard< std::mutex > lock( currentlySettlingPaymentsMutex_ );
...
variant< SettlementResponse, HttpError > PaymentManager::checkPaymentIsNewAndSettleIt(
    ...
    try {
        ...
        return checkPaymentIsNewAndSettleItUnsafe( networkConfig, resource, organization,
            paymentPayload,
            ipAddress );
    } catch (const std::exception& e) {
...
variant< SettlementResponse, HttpError > PaymentManager::decodeValidateAndSettlePayment(
    ...
    try {
        ...
        return checkPaymentIsNewAndSettleIt( *config.network(), resource, organization,
            paymentPayload,
            ipAddress );
    } catch (std::exception& e) {
        spdlog::error( "decodeValidateAndSettlePayment had exception  {}", e.what() );
        return HttpError( ERR_INTERNAL_SERVER_ERROR,
            std::string( "decodeValidateAndSettlePayment had exception" )
            + e.what() );
    }
}
