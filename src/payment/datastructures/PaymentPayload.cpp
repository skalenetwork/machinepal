...
PaymentPayload::PaymentPayload(int x402Version, const std::string &scheme, const std::string &network,
...
std::optional<HttpError> PaymentPayload::validateAndVerifySignature(const MachinePayConfig &config,
...
std::string PaymentPayload::createHttpHeaderValue() {
    ...
    return "X-PAYMENT: " + URLUtils::base64Encode(paymentJson.dump());
}