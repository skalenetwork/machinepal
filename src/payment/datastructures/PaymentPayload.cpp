...
std::shared_ptr< PaymentPayload > PaymentPayload::fromJson( const json& j ) {
    try {
        ...
        CHECK_STATE_JSON( j.contains("scheme"),
            "Missing required field 'scheme' in PaymentPayload JSON", j );
        CHECK_STATE_JSON( j.contains("network"),
            "Missing required field 'network' in PaymentPayload JSON", j );
        CHECK_STATE_JSON( j.contains("payload"),
            "Missing required field 'payload' in PaymentPayload JSON", j );

        CHECK_STATE_JSON( j.at("x402Version").is_number_integer(),
            "'x402Version' must be an integer in PaymentPayload JSON", j );
        CHECK_STATE_JSON( j.at("scheme").is_string(),
            "'scheme' must be a string in PaymentPayload JSON", j );
        CHECK_STATE_JSON( j.at("network").is_string(),
            "'network' must be a string in PaymentPayload JSON", j );
...
std::optional< HttpError > PaymentPayload::validateAndVerifySignature(
    ...
    try {
        ...
        if (network_ != config.network()->name()) {
            return HttpError( ERR_BAD_REQUEST,
                std::string( "Payment network does not match configured network " ) +
...
std::optional< HttpError > PaymentPayload::verifyEIP3009Signature( const MachinePayConfig& config,
    ...
    try {
        ...
        return HttpError( ERR_INTERNAL_SERVER_ERROR,
            "Exception verifying EIP3009 signature of payment payload" );
    }
}
