...
bool BackendConnection::proxyToBackEndOptions(const std::string &requestBody, std::string &backendResponseBody,
    std::string &errorMessage) {
    (void)requestBody; // OPTIONS typically has no body
    static thread_local std::unique_ptr< CURL, decltype( &curl_easy_cleanup ) > curlThreadLocal(
        nullptr, &curl_easy_cleanup );
