...
uint256_t Encoding::u256FromHexOrDecimal(const std::string &str) {
    if (str.empty()) throw std::invalid_argument("Empty input for uint256");
    if (std::any_of(str.begin(), str.end(), ::isspace)) throw std::invalid_argument("Whitespace not allowed in uint256 input");
    if (str[0] == '-') throw std::invalid_argument("Negative numbers not allowed for uint256");

    if (str.size() >= 2 && str[0] == '0' && (str[1] == 'x' || str[1] == 'X')) {
        auto bytes = fromHex(str);
        if (bytes.empty()) throw std::invalid_argument("Empty hex input for uint256");
        if (bytes.size() > 32) throw std::invalid_argument("Hex input too long for uint256");
        uint256_t val = 0;
        for (auto b : bytes) {
            val = (val << 8) | b;
        }
...
std::array<uint8_t, 32> Encoding::fromHexToHash(const std::string &hexStr) {
    ...
    if (bytes.size() > 32) throw std::invalid_argument("Hex input too long for array32");
