...
EthPublicKey::EthPublicKey(std::span<const uint8_t, 64> bytes) { std::copy(bytes.begin(), bytes.end(), bytes_.begin()); }
...
EthPublicKey EthPublicKey::parseFlexible(const std::string& hex) {
    auto v = hexToBytesFlexible(hex);
    std::array<uint8_t,64> arr{}; std::copy(v.begin(), v.end(), arr.begin());
