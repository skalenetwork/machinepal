...
static std::vector<uint8_t> hexToBytesFlexible(const std::string& hex)
{
    ...
    if (s.size() != 130 || s.substr(0, 2) != "04")
        throw std::invalid_argument(
            "Public key hex must be 130 characters (0x04 + 64 bytes); got " + std::to_string(s.size()));
    for (char c : s) if (!std::isxdigit(static_cast<unsigned char>(c))) throw std::invalid_argument(
        "Invalid hex character in public key");
...
{
    if (len != 64) throw std::invalid_argument("EthPublicKey must be 64 bytes");
...
EthPublicKey::EthPublicKey(const std::string& hex)
{
    ...
    if (!isValid(bytes_)) throw std::invalid_argument("Invalid public key");
}
...
EthPublicKey EthPublicKey::parseFlexible(const std::string& hex)
{
    ...
    if (!isValid(arr)) throw std::invalid_argument("Invalid public key");
