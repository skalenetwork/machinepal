...
EIP712Signature::EIP712Signature(const std::string& hex)
{
    if (hex.size() > 132)
    {
        throw std::invalid_argument(
            "Hex string too long to be a valid signature: " + std::to_string(hex.size()) + " chars "
            + hex);
    }
    std::string s = hex;
...
EIP712Signature::EIP712Signature(const std::array<uint8_t, 32>& r, const std::array<uint8_t, 32>& s, uint8_t v)
{
