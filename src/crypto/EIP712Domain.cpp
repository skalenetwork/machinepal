...
) : name_(name), version_(version), chainId_(chainId), verifyingContract_(verifyingContract) {
}
...
std::array<uint8_t, 32> EIP712Domain::hashDomain() const {
    ...
    encoded_data.insert(encoded_data.end(), EIP712_DOMAIN_TYPEHASH.begin(), EIP712_DOMAIN_TYPEHASH.end());

    // EIP-712 field: name (string)
    auto hashed_name = keccak::keccak256(name_);
    encoded_data.insert(encoded_data.end(), hashed_name.begin(), hashed_name.end());

    // EIP-712 field: version (string)
    auto hashed_version = keccak::keccak256(version_);
    encoded_data.insert(encoded_data.end(), hashed_version.begin(), hashed_version.end());
    ...
    if (chainIdBytes.size() < 32) {
        ...
        encoded_data.insert(encoded_data.end(), padding.begin(), padding.end());
    }
    encoded_data.insert(encoded_data.end(), chainIdBytes.begin(), chainIdBytes.end());
    ...
    encoded_data.insert(encoded_data.end(), paddedContract.begin(), paddedContract.end());



    return keccak::keccak256(encoded_data);
}
