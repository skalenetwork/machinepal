...
struct DiscoveredResource
{
    ...
    {
        return resource == other.resource && type == other.type && x402Version == other.x402Version && accepts == other.
            accepts && lastUpdated == other.lastUpdated && metadata == other.metadata;
    }

    friend std::ostream& operator<<(std::ostream& os, const DiscoveredResource& r)
    {
        os << "{resource: " << r.resource << ", type: " << r.type << ", x402Version: " << r.x402Version <<
            ", accepts: [";
        for (const auto& a : r.accepts) os << a << ", ";
        os << "], lastUpdated: " << r.lastUpdated << ", metadata: " << (r.metadata
                                                                            ? *r.metadata
