...
EasyNetFacilitatorClient::EasyNetFacilitatorClient( EasyNetDb& database, EthAddress& assetAddress,
    u256& chainId )
    : db_( database ),
...
nlohmann::json EasyNetFacilitatorClient::verify(
    ...
    try {
        ...
        auto payload = verifyCore( paymentPayloadJson, paymentRequirementsJson, fromWalletAddress,
            error );
        if (error) {
...
nlohmann::json EasyNetFacilitatorClient::settle(
    ...
    try {
        EthAddress fromWalletAddress;
        optional< string > error;
        auto paymentPayload = verifyCore( paymentPayloadJson, paymentRequirementsJson,
            fromWalletAddress, error );
        EthAddress toWalletAddress = paymentPayload->payload()->authorization()->to();
        ...
        auto result = db_.transferValue( fromWalletAddress, toWalletAddress, assetWalletAddress,
            transferValue );
