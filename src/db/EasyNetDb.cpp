...
EasyNetDb::EasyNetDb( MachinePayApp& app, DbType type,
    const std::optional< std::string >& connectionInfo )
...
void EasyNetDb::newWallet( const EthAddress& walletAddress, const EthAddress& assetAddress,
...
EasyNetDb::TransferResult EasyNetDb::transferValue( const EthAddress& fromAddress,
    ...
    try {
        ...
        databaseSession <<
            "SELECT value FROM state WHERE walletAddress = :walletAddress AND assetAddress = :assetAddress"
            ,
        ...
        if (senderBalanceIndicator == soci::i_null || senderBalanceValueStringFromDatabase.
            empty()) {
            RETHROW_NESTED2(
                "Sender wallet state row not found for walletAddress=" +
                fromWalletAddressDatabaseString +
                ", assetAddress=" + assetContractAddressDatabaseString );
        }

        u256 senderCurrentBalanceValue = Encoding::u256FromHexOrDecimal(
            senderBalanceValueStringFromDatabase );
        if (transferAmountValue > senderCurrentBalanceValue) {
            logger_->trace(
                "transferValue: insufficient funds walletAddress={} assetAddress={} have={} need={}",
                fromWalletAddressDatabaseString,
        ...
        bool receiverStateRowExists = !(
            receiverBalanceIndicator == soci::i_null || receiverBalanceValueStringFromDatabase.
            empty() );
        if (receiverStateRowExists) {
            receiverCurrentBalanceValue = Encoding::u256FromHexOrDecimal(
                receiverBalanceValueStringFromDatabase );
        }
        ...
        if (transferAmountValue > maxUint256Value - receiverCurrentBalanceValue) {
            logger_->trace(
                "transferValue: overflow would occur receiverWalletAddress={} assetAddress={} receiverBalance={} "
        ...
        std::string senderUpdatedBalanceDecimalString = Encoding::u256ToDecimal(
            senderUpdatedBalanceValueAfterTransfer );
        std::string receiverUpdatedBalanceDecimalString = Encoding::u256ToDecimal(
            receiverUpdatedBalanceValueAfterTransfer );

        // 4. Persist changes
        databaseSession <<
            "UPDATE state SET value = :value WHERE walletAddress = :walletAddress AND assetAddress = :assetAddress"
            ,
        ...
        if (receiverStateRowExists) {
            ...
            databaseSession <<
                "INSERT INTO state (walletAddress, assetAddress, value) VALUES (:walletAddress, :assetAddress, :value)"
                ,
                soci::use( toWalletAddressDatabaseString, "walletAddress" ),
                soci::use( assetContractAddressDatabaseString, "assetAddress" ),
                soci::use( receiverUpdatedBalanceDecimalString, "value" );
        ...
        logger_->trace(
            "transferValue success: fromWalletAddress={} toWalletAddress={} assetContractAddress={} transferAmount={} senderUpdatedBalance={} receiverUpdatedBalance={}",
            fromWalletAddressDatabaseString,
            toWalletAddressDatabaseString,
            assetContractAddressDatabaseString,
            Encoding::u256ToDecimal( transferAmountValue ),
...
void EasyNetDb::fundUserWalletWithFundsIfNewWallet( const EthAddress& walletAddress,
    const EthAddress& assetAddress ) {
    std::unique_lock< std::shared_mutex > stateMutexUniqueLock( stateMutex_ );
    ...
    try {
        ...
        if (!( existingValueIndicator == soci::i_null || existingValueStringFromDatabase.
               empty() )) {
            logger_->trace(
                "fundUserWalletWithFundsIfNewWallet: wallet already funded walletAddress={} assetAddress={} existingValue={}",
                walletAddressDatabaseString, assetContractAddressDatabaseString,
        ...
        u256 initialFundingAmountValue = Encoding::u256FromHexOrDecimal(
        ...
        logger_->trace(
            "fundUserWalletWithFundsIfNewWallet: funded new wallet walletAddress={} assetAddress={} initialValue={}",
            walletAddressDatabaseString, assetContractAddressDatabaseString,
...
std::optional< u256 > EasyNetDb::getBalance( const EthAddress& walletAddress,
    const EthAddress& assetAddress ) const {
